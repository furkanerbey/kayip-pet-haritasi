...
splash.addEventListener('click', () => {
  splash.classList.add('fade');          // yumuşak kaybol
...
function showToast(msg, type = "") {
  ...
  toast.className = `toast show ${type}`;    // type = "success" | "error" | ""
}
function hideToast() {
...
const SUPABASE_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJnZnBhZmx1Y3p5eHJ2YmZ2aWpnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTAzMjYyNDAsImV4cCI6MjA2NTkwMjI0MH0.IJiNbVTDjZeOyvaIeYvQJRE4EHbxPdVHF46DdKV74-s";
...
/* Yardımcı */
const setSend = (txt, dis) => { sendTxt.textContent = txt; sendBtn.disabled = dis; };
...
/* ── “Konumum” butonu ── */
locBtn.onclick = () => {
  if (!navigator.geolocation) return showToast("Tarayıcınız konum desteklemiyor", "error");

  if (location.protocol !== "https:" &&
      location.hostname !== "localhost" &&
      location.hostname !== "127.0.0.1") {
    return showToast("Konum için HTTPS veya localhost açın", "error");
  }
  ...
  navigator.geolocation.getCurrentPosition(
    pos => {
      ...
      selectLocation({ lat, lng: lon });          // otomatik de seç
    },
    () => showToast("Konum alınamadı / izin yok", "error"),
...
function addReport(r) {
  const m = L.marker([r.lat, r.lon])
    .bindPopup(`<img src="${r.photourl}" width="130">`);
  m.on("click", () => showModal(r));
  cluster.addLayer(m);
  reports.unshift(r);
}
(async () => {
